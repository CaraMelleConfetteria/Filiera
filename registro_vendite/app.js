...
(function () {
  ...
  /* La PT-210 non "vede" la carta: la sente con un sensore ottico che spara
     infrarosso e legge quanto torna indietro. Certe carte economiche hanno
     una patina che l'infrarosso se lo mangia: la stampante conclude che
     carta non ce n'è, suona e si rifiuta di stampare — col rotolo pieno
     davanti. Si riconosce da un dettaglio: girando il rotolo al contrario
     smette di suonare (il retro riflette) ma non scrive (il calore agisce
     solo sul lato trattato).
     Questi due comandi le dicono di smettere di guardare il sensore.

     QUELLO CHE COSTA: da qui in poi non avvisa più quando la carta finisce
     DAVVERO. Si stampa nel vuoto e lo scontrino si rifà. Col rotolo
     originale, che il sensore riconosce benissimo, conviene rimettere
     false. */
  var IGNORA_SENSORE_CARTA = true;

  function comandiSensore(push) {
    if (!IGNORA_SENSORE_CARTA) return;
    push(new Uint8Array([ESC, 0x63, 0x33, 0x00]));  // niente segnale di fine carta
    push(new Uint8Array([ESC, 0x63, 0x34, 0x00]));  // non fermarti per il sensore
  }
